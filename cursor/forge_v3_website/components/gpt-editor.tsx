...
export function GPTEditor({ generatedPrompt, onEditComplete }: GPTEditorProps) {
  ...
  return (
    <Card className="bg-black/50 border border-lead-gray/30 backdrop-blur-sm p-6 shadow-lead hover:shadow-lead-hover">
      <div className="flex items-center justify-between mb-4">
        ...
        {editResult && (
          <Badge variant="skre-gold" className="bg-black/50 border border-lead-gray/30 backdrop-blur-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <Select value={options.focus} onValueChange={(value: any) => setOptions({ ...options, focus: value })}>
              <SelectTrigger className="bg-black/50 border border-lead-gray/30 backdrop-blur-sm">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        <Button onClick={handleOptimizePrompt} disabled={!generatedPrompt || isEditing} className="shadow-gold hover:shadow-gold-hover">
          {isEditing ? (
            <>
        ...
        {editResult && (
          <>
            <Button variant="skre-outline" onClick={handleCopyEdited} className="bg-black/50 border border-lead-gray/30 backdrop-blur-sm bg-transparent">
              <Copy className="w-4 h-4 mr-2" />
              Copy Optimized
            </Button>

            <Button variant="skre-outline" onClick={handleDownloadEdited} className="bg-black/50 border border-lead-gray/30 backdrop-blur-sm bg-transparent">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          <h4 className="font-semibold text-foreground mb-2 flex items-center gap-2">
            Original
            {generatedPrompt && (
              <Badge variant="skre-outline" className="text-xs">
          ...
          <Textarea
            ...
            className="min-h-[400px] font-mono text-sm bg-black/50 border border-lead-gray/30 backdrop-blur-sm"
            readOnly
          />
        </div>
        ...
        <div>
          ...
          <Textarea
            ...
            className="min-h-[400px] font-mono text-sm bg-black/50 border border-lead-gray/30 backdrop-blur-sm border-primary/50"
            readOnly
          />
        </div>
      </div>
