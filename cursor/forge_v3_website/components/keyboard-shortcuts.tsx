  ...
  if (!showShortcuts) {
    return (
      <Button
        ...
        className="fixed bottom-4 right-4 bg-black/50 border border-lead-gray/30 backdrop-blur-sm z-50"
      ...
      <Card className="glass-strong p-6 max-w-md w-full animate-slide-up">
            ...
            <div key={index} className="flex items-center justify-between">
              ...
              <div className="flex gap-1">
                {shortcut.keys.map((key, keyIndex) => (
                  <Badge key={keyIndex} variant="skre-outline" className="text-xs font-mono">
                    {key}
                  </Badge>
                ))}
              </div>
        ...
        <div className="mt-4 pt-4 border-t border-border">
          <p className="text-xs text-muted-foreground text-center">
            Press{" "}
            <Badge variant="skre-outline" className="text-xs font-mono mx-1">
              Esc
            </Badge>{" "}
            to close
          </p>
