...
export function ModuleCard({ module, isSelected, onSelect, onViewDetails }: ModuleCardProps) {
  ...
  return (
    <Card
      className={`bg-black/50 border border-lead-gray/30 backdrop-blur-sm p-4 cursor-pointer transition-all duration-300 hover:shadow-gold hover:shadow-gold-hover border-l-4 ${
        isSelected ? "ring-2 ring-primary shadow-gold hover:shadow-gold-hover border-l-primary" : `border-l-${vectorColor.split("-")[1]}-500`
      }`}
