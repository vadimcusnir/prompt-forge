...
/**
 * Fix All Button Components Script
 * 
 * This script finds and replaces ALL Button components with onClick handlers
...
// Function to find and replace all Button components with onClick
function replaceAllButtonWithOnClick(content) {
  let newContent = content;
  let replaced = false;
  
  // Pattern to match Button components with onClick (more comprehensive)
  const buttonPattern = /<Button\s+([^>]*?)onClick\s*=\s*\{([^}]+)\}([^>]*?)>([\s\S]*?)<\/Button>/g;
  
  newContent = newContent.replace(buttonPattern, (match, beforeOnClick, onClickValue, afterOnClick, buttonContent) => {
    replaced = true;
...
function processFile(filePath) {
  try {
    ...
    const { content: newContent, replaced } = replaceAllButtonWithOnClick(content);
...
// Main execution
async function main() {
  console.log('🔧 Fixing ALL Button components with onClick handlers...\n');
